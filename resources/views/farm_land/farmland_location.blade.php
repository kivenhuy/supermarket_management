...
$('#dimmer').dimmer('show');
  ...
  function initMap($data='',$key = 0) 
  {
    ...
    // console.log(position);
      ...
      const marker = new google.maps.Marker({
        position: new google.maps.LatLng(position['farm_land_lat_lng'][0]['lat'], position['farm_land_lat_lng'][0]['lng']),
        content: pinGlyph.element,
      });

      var myTrip = new Array();
      // if((position['farm_land_lat_lng']).length > 0)
      // {
        
      //   for (j = 0; j < position['farm_land_lat_lng'].length; j++) { 
      //     console.log(position['farm_land_lat_lng'][j]['lat']);
          
      //     myTrip.push(new google.maps.LatLng(position['farm_land_lat_lng'][j]['lat'], position['farm_land_lat_lng'][j]['lng']));
      //   }
      //   myTrip.push(new google.maps.LatLng(position['farm_land_lat_lng'][0]['lat'], position['farm_land_lat_lng'][0]['lng']));
      // }
      ...
      const content = document.createElement("div");
          ...
          content.innerHTML = `
              ...
              <div class="name">
                <div>
                  <p>${position['farmer_details']['full_name']}</p>
                </div>
              </div>
  ...
  // function fireNewFunctionWhenComplete()
  // {
    // $('#dimmer').dimmer('off');
  // }
</script>
